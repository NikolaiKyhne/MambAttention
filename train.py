...
def train(rank, args, cfg):
    ...
    generator = DistributedDataParallel(generator, device_ids=[rank],broadcast_buffers=True).to(device)
    discriminator = DistributedDataParallel(discriminator, device_ids=[rank],broadcast_buffers=True).to(device)
...
def main():
    ...
    # Print GPU info and set up `torchrun` for distributed training
    ...
    # Directly call train, as `torchrun` handles parallel execution
